 ...
 private:
  ...
  Block* try_allocate_expandable_block(
    ...
    while (candidate->size < size) {
      ...
      // A single allocation cannot span expandable segments. If the stream's
      // (possibly downsized) per-segment reserve is smaller than this single
      // allocation, no segment can ever satisfy it: the segment's address space
      // is exhausted (next == null) before we reach `size`. Fail with an
      // actionable message instead of dereferencing a null next block.
      if (new_candidate == nullptr) {
        ...
        TORCH_CHECK_WITH(
            ...
            format_size(candidate->expandable_segment_->size()),
