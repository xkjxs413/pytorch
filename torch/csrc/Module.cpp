...
:func:`torch.set_num_threads` onto the new thread.
  ...
  py_module.def(
      ...
      [](const py::object& converter,
        ...
        // The fake tensors this mode creates each hold a strong reference
        // instead (FakeTensorMode::pin_pyobj), so the Python mode outlives them.
  ...
  // see [in_kernel_invocation]: makes cpp fake tensors report their meta backing
  // device for the duration of the scope, so Python fake impls that run meta
  // kernels behave the same as the C++ fallback's meta dispatch.
