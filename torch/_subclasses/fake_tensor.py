...
class CppFakeTensorMode:
    ...
    def from_tensor(
        ...
        *,
        ...
        # all-static symbolic context (rather than dropping the ShapeEnv). This
        # must apply even without a source (e.g. freezing constants fakeified by
        # fake_tensor_prop); otherwise the live ShapeEnv allocates symbolic sizes
        # with a placeholder source that later breaks guard production.
        if static_shapes and symbolic_context is None:
            from torch.fx.experimental.symbolic_shapes import (
                DimDynamic,
                StatelessSymbolicContext,
            )

            # A view's base is meta-converted with the live ShapeEnv too, so it
            # needs a matching all-static context; otherwise the base (which has
            # no source) gets a fresh symbolic size that later breaks guard
            # production. Recurse so nested view bases stay static as well.
            def static_context(t: Tensor) -> StatelessSymbolicContext:
                base_context = (
                    static_context(t._base)
                    if t._is_view() and t._base is not None
                    else None
                )
                return StatelessSymbolicContext(
                    dynamic_sizes=[DimDynamic.STATIC] * t.dim(),
                    view_base_context=base_context,
                )

            symbolic_context = static_context(tensor)
    ...
    def in_kernel_invocation_manager(self) -> Generator[None, None, None]:
        ...
        with torch._C._PreserveDispatchKeyGuard(), torch._C._FakeInKernelInvocation():
